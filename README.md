...
The [`demo/`](demo/) folder is a complete run on a fictional candidate: a mid-level backend engineer applying for a Senior Backend Engineer role, with an average CV and real gaps. You can see exactly what CV Coach produces without running anything. Open the dashboard first, then compare the input CV to the rewrite.

CV Coach scored that candidate **63 / 100 (NEEDS WORK)** and returned a hiring-manager verdict of **weak_maybe**, calling out the missing architecture-ownership and mentoring signal, the absent metrics, and an unexplained employment gap, then named the single highest-leverage fix. Full write-up in [demo/README.md](demo/README.md).
